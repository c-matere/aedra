...
async function runScenario(name: string, messages: string[], expectedKeywords: string[][]) {
    console.log(`\n\x1b[36m=== Scenario: ${name} ===\x1b[0m`);

    const user = await prisma.user.findFirst({ where: { role: UserRole.SUPER_ADMIN } });
    ...
    for (let i = 0; i < messages.length; i++) {
        ...
        try {
            ...
            console.log(`\x1b[32mAI:\x1b[0m ${response.substring(0, 500)}${response.length > 500 ? '...' : ''}`);

            const foundKeywords = keywords.filter(k => response.toLowerCase().includes(k.toLowerCase()));
            if (foundKeywords.length === keywords.length) {
                console.log(`\x1b[32m✓ Passed (Keywords found: ${foundKeywords.join(', ')})\x1b[0m`);
            } else {
                const missing = keywords.filter(k => !response.toLowerCase().includes(k.toLowerCase()));
...
async function main() {
    ...
    await runScenario("Robust Context Resolution (Zero-Context Start)",
        [
    ...
    await runScenario("Sticky Routing & Workflow Initiation",
        [
    ...
    await runScenario("Super Admin Cross-Company Search",
        [
            "who is Sarah Ali?",
            "create an invoice for her rent for 1500"
        ],
        [
            ["Sarah Ali", "tenant"],
