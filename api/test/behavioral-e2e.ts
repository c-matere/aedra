...
async function runStep(token: string, msg: string, history: any[], chatId?: string) {
...
async function main() {
    ...
    try {
        ...
        const step3 = await runStep(token, "I am interested in company 4", history, chatId);
        history = step3.history;

        // Step 4: Add Property
        const step4 = await runStep(token, "add a property called 'Simulated Growth Tower' in downtown Nairobi in this company", history, chatId);
        ...
        // Step 5: Add Tenant
        const step5 = await runStep(token, "add a tenant called 'John Behavioral' to this new property", history, chatId);
        ...
        const step6 = await runStep(token, "delete the tenant John Behavioral", history, chatId);
        history = step6.history;

        // Step 7: Reports
        const step7 = await runStep(token, "generate a financial report for this company", history, chatId);
