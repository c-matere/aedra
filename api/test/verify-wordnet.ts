...
async function verify() {
  ...
  const testCases = [
    ...
    { msg: "generate mckinsey report", expected: "generate_mckinsey_report", route: "DIRECT" },
    { msg: "tengeneza ripoti", expected: "generate_mckinsey_report", route: "DIRECT" },
    ...
    { msg: "I am interested in alphask", expected: "select_company", route: "HINT" },
  ];
  ...
  for (const tc of testCases) {
    ...
    if (isIntentMatch && isRouteMatch) {
      console.log(`✅ PASS: "${tc.msg}" -> ${result.intent} (${result.route}, conf: ${result.confidence.toFixed(2)})`);
      passed++;
    } else {
      console.log(`❌ FAIL: "${tc.msg}"`);
      console.log(`   Expected: ${tc.expected} (${tc.route})`);
      console.log(`   Actual:   ${result.intent} (${result.route}, conf: ${result.confidence.toFixed(2)})`);
    }
  }
