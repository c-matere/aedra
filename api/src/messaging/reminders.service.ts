...
export class RemindersService {
  ...
  async sendBulkReminders(actor: AuthenticatedUser, propertyId?: string) {
    this.logger.log(`Starting bulk reminder process for company ${actor.companyId}`);

    // STEP 2 & 3 — Build recipient list using portfolio snapshot
    const snapshot = await this.unitsService.getPortfolioSnapshot(actor, propertyId);
    ...
    for (const [propId, data] of Object.entries(snapshot)) {
      ...
      for (const unit of d.unpaid_this_month) {
        ...
        const tenant = await this.prisma.tenant.findUnique({
            where: { id: unit.tenantId || (await this.findTenantId(unit.number, propId)) },
            select: { phone: true, firstName: true, language: true }
        });

        if (!tenant || !tenant.phone) continue;

        const message = this.generateReminderMessage(tenant, unit.number, balance, tone);
        ...
        try {
            ...
            totalSent++;
            results.push({ tenant: tenant.firstName, unit: unit.number, status: 'SENT' });
        } catch (err) {
            this.logger.error(`Failed to send reminder to ${tenant.phone}: ${err.message}`);
            results.push({ tenant: tenant.firstName, unit: unit.number, status: 'FAILED', error: err.message });
        }
      }
    }
  ...
  private generateReminderMessage(tenant: any, unitNumber: string, balance: number, tone: 'GENTLE' | 'FIRM' | 'URGENT') {
