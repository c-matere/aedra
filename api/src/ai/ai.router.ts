...
const ROUTER_MODEL = (process.env.GEMINI_MODEL || '').trim() || 'gemini-2.5-flash';
const GROQ_ROUTER_MODEL = 'llama-3.3-70b-versatile';
...
export const selectModelKey = async (
    ...
    if (text.startsWith('/write')) return { intent: 'write', reason: 'Direct command override' };
    if (text.startsWith('/report')) return { intent: 'report', reason: 'Direct command override' };
    if (text.startsWith('/read')) return { intent: 'read', reason: 'Direct command override' };
    ...
    const writeHints = [
        ...
        // Swahili write hints
        'unda', 'ongeza', 'rekodi', 'badilisha', 'hariri', 'andika', 'rekebisha', 'panga',
        'timiza', 'weka', 'mpya', 'pili', 'matengenezo', 'vujisha', 'choo', 'bomoka', 'haribika'
    ];
    const reportHints = [
        ...
        'ripoti', 'muhtasari', 'uchambuzi', 'mapato', 'matumizi', 'faida', 'hasara', 'mkusanyiko',
        'deni', 'madeni', 'kifedha', 'utendaji', 'kila mwezi', 'robo mwaka', 'mwaka', 'tuma'
    ];

    if (writeHints.some((h) => text.includes(h))) return { intent: 'write', reason: 'Lexical match (write hints)' };
    if (reportHints.some((h) => text.includes(h))) return { intent: 'report', reason: 'Lexical match (report hints)' };
    ...
    if (history && history.length > 0) {
        ...
        const aiAskingForWriteDetail = [
            'confirm', 'proceed', 'sure', 'priority', 'company', 'category', 'status', 'due date', 'amount'
        ...
        if (aiAskingForWriteDetail || hasRecentWriteIntent) {
            if (!reportHints.some(h => text.includes(h))) {
                return { intent: 'write', reason: 'Contextual fallback (AI follow-up or recent write activity)' };
            }
        }
    }
    const commonFollowUps = [
        ...
        'ndio', 'sawa', 'endelea', 'fanya', 'thibitisha', 'ndiyo', 'izidi'
    ];

    if (commonFollowUps.some(f => text === f || text.startsWith(f + ' ')) && history.length > 0) {
        return { intent: 'write', reason: 'Affirmation / Confirmation follow-up' };
    }

    // --- LLM classification as last resort ---
    try {
        const safeHistory = (history || []).slice(-5).map(h => {
            const role = typeof h?.role === 'string' ? h.role : 'user';
            const content = typeof h?.content === 'string'
                ? h.content
                : (typeof h?.message === 'string' ? h.message : '');
            return `${role}: ${content}`;
        }).join('\n');
        ...
        const runClassification = async () => {
            ...
            if (groq) {
                try {
                    ...
                    console.warn(`[Router] Groq failed, falling back to Gemini... ${e.message}`);
                }
            }
        ...
        try {
            ...
            if ((intent === 'read' || intent === 'write' || intent === 'report') && confidence > 0.6) {
                return { intent, reason: data.reason || 'LLM classified with high confidence' };
            }
        } catch (jsonErr: any) {
            console.warn('Router JSON parse failed:', jsonErr.message, raw?.slice(0, 120));
        }
    } catch (error: any) {
        console.warn('AI Router classification failed, falling back to default:', error.message);
    }

    return { intent: 'read', reason: 'Defaulting to read (low confidence / no strong cues)' };
};
