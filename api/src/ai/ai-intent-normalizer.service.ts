...
export class AiIntentNormalizerService {
  ...
  private readonly UNIT_PATTERN = /\b(?:unit|house|nyumba|room|#)\s*([a-z0-9]+)\b/i;
  private readonly AMOUNT_PATTERN = /\b(?:ksh|kes|sh)?\s*(\d{1,3}(?:,\d{3})*(?:\.\d+)?)\s*(?:k|thousand)?\b/i;
  ...
  normalize(message: string): NormalizationHint {
    ...
    if (amountMatch) {
      ...
      if (text.includes(`${amountMatch[1]}k`) || text.includes(`${amountMatch[1]} k`)) {
        numericVal *= 1000;
      }
    ...
    // 3. Emergency Signals
    if (urgency !== 'EMERGENCY' && /\b(urgent|emergency|dharura|haraka|critical|immediately)\b/i.test(text)) {
      urgency = 'HIGH';
    }

    // 4. HARD PATTERN RULES (Deterministic Overrides)
    if (/\b(maji|imepotea|hakuna maji|bomba|imepasuka|leaking|leak|water issue)\b/i.test(text)) {
    ...
    if ((text.includes('weka') || text.includes('register') || text.includes('add')) && 
    ...
    return {
      ...
      recommendedAction: urgency === 'EMERGENCY' ? 'LOG_MAINTENANCE_IMMEDIATELY' : undefined
    };
  }
}
