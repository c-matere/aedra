...
describe('AiService Staging Tools', () => {
  ...
  describe('assemble_report_staged', () => {
    it('should assemble report from multiple staged data sources', async () => {
      const tenants = [
        { unitIdentifier: '1A', missedPayments: 1, lateRate: 0.1 },
      ];
      const payments = [{ amount: 1000 }];
      const invoices = [{ amount: 1100 }];

      mockStaging.retrieve.mockImplementation((jobId: string, key: string) => {
      ...
      mockReportsGenerator.generatePremiumPdf.mockResolvedValue(
        'http://report.url',
      );

      const result = await (aiService as any).executeTool(
        'assemble_report_staged',
        {
          jobId: 'job123',
        },
        { userRole: UserRole.COMPANY_ADMIN },
