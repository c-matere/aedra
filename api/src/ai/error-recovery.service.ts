...
export class ErrorRecoveryService {
  ...
  buildErrorRecovery(
    ...
    const maskedUserId = context.userId ? `${context.userId.substring(0, 4)}...${context.userId.substring(context.userId.length - 4)}` : 'anon';
    this.logger.error(`Action ${action} failed: ${error.message}`, error.stack, {
      userId: maskedUserId,
      action
    });
    ...
    const messages = (this.RECOVERY_MESSAGES as any)[action] || this.RECOVERY_MESSAGES.default;
