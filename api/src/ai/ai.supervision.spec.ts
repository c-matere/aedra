...
describe('AiService Supervision & Self-Consistency', () => {
    ...
    beforeEach(() => {
        mockPrisma = {
            chatMessage: { create: jest.fn(), updateMany: jest.fn().mockResolvedValue({ count: 1 }) },
        };
        ...
        aiService = {
            ...
            sampleStructuredOutput: (AiService.prototype as any).sampleStructuredOutput,
    ...
    describe('detectSkill (Dynamic)', () => {
        it('should detect rent_reminder by key overlap', () => {
            const json = JSON.stringify({ status: 'PAID', amount_due: 100, last_payment_date: '2023-01-01' });
        ...
        it('should detect generate_mckinsey_report by key overlap', () => {
            const json = JSON.stringify({ report_url: 'http://link.pdf', key_finding: 'Revenue is up' });
            const skillId = (aiService as any).detectSkill(json);
    ...
    describe('runSupervisedLoop', () => {
        it('should return pipeline output if critic passes', async () => {
            ...
            mockPipeline.processResponse.mockResolvedValue({ success: true, output: 'Formatted Response' });

            const result = await (aiService as any).runSupervisedLoop('check_rent_status', '{}', 'context', {});
        ...
        it('should attempt correction if critic fails once', async () => {
            // First pass: Fail
            mockCritic.evaluate.mockResolvedValueOnce({ pass: false, feedback: ['Mismatch'] })
                // Second pass: Pass
            ...
            const mockChat = {
                sendMessage: jest.fn().mockResolvedValue({
                    response: {
                        candidates: [{ content: { parts: [{ text: 'JSON_STRUCTURED_OUTPUT: {"status":"PAID"}' }] } }]
                    }
                })
            };

            mockPipeline.processResponse.mockResolvedValue({ success: true, output: 'Corrected Response' });

            const result = await (aiService as any).runSupervisedLoop('check_rent_status', '{}', 'context', mockChat);
