...
export async function tryDirectTool(
  ...
  executeTool: (name: string, args: any, context: any, role?: string, language?: string) => Promise<any>,
  language: string = 'en',
  cache: Cache,
): Promise<any | null> {
  ...
  const balanceRegex = /^(balance|deni|balansi|nadhai unanilinda kiasi gani|deni langu|kiasi gani|how much|amount due)$/i;
  ...
  const profileRegex = /^(my profile|info yangu|wasifu|details zangu|who am i|mimi ni nani)$/i;
  ...
  const listCompaniesRegex = /^(list companies|orodha ya makampuni|makampuni yote)$/i;
