...
type ActionTemplate =
    | ((language: Lang) => { greeting: string; subtitle: string; actions: { key: string; label: string; }[] })
    | Record<Lang, (name: string, ctx: any) => { greeting: string; subtitle: string; actions: { key: string; label: string; }[] }>;
...
const GREETING_TEMPLATES: Record<QuickRole, Record<Lang, (name: string, ctx: any) => { greeting: string; subtitle: string; actions: { key: string; label: string; }[] }>> = {
    SUPER_ADMIN: {
        EN: (name: string) => ({
    ...
    TENANT: {
        EN: (name: string, ctx: TenantContext = { balanceDue: 0, currentMonth: '' }) => ({
            greeting: `👋 Hi ${name}.`,
            subtitle: ctx.balanceDue > 0
        ...
        SW: (name: string, ctx: TenantContext = { balanceDue: 0, currentMonth: '' }) => ({
    ...
    LANDLORD: {
        EN: (name: string, ctx: LandlordContext = { portfolioName: '', collectionRate: 0, vacantCount: 0 }) => ({
            greeting: `👋 Good ${timeOfDay()}, ${name}.`,
        ...
        SW: (name: string, ctx: LandlordContext = { portfolioName: '', collectionRate: 0, vacantCount: 0 }) => ({
            greeting: `👋 ${swahiliGreeting()}, ${name}.`,
...
const greetableRegex = /^(hi|hello|habari|mambo|hey|jambo|menu|start|help|msaada|sasa|bonjour)/i;
...
const renderActionMenu = (role: QuickRole, language: Lang, context: any) => {
    ...
    const payload = template(name, {
        ...
        collectionRate: context?.collectionRate || context?.metrics?.collectionRate || 0,
        vacantCount: context?.vacantCount || context?.metrics?.vacantCount || 0,
    });
    const actionLines = payload.actions.map(a => `${a.key}. ${a.label}`).join('\n');
    return [
        payload.greeting,
        payload.subtitle,
        '',
...
const loadSession = async (cacheManager: Cache | undefined, context: any): Promise<SessionState | null> => {
...
const saveSession = async (cacheManager: Cache | undefined, context: any, state: SessionState) => {
...
const POST_ACTION_SUGGESTIONS: Record<string, (lang: Lang, ctx: any) => { text: string; nextAction: string; options?: Record<string, string> } | null> = {
...
const executeQuickAction = async (
    ...
    switch (action) {
        ...
        case 'switch_company':
            ...
            return executeTool('list_companies', {}, context);
        case 'RETRY':
            return { success: true, data: lang === 'SW' ? 'Tafadhali jaribu tena ombi lako.' : 'Please try your request again.', action: 'retry' };
        case 'MAIN_MENU':
            return { success: true, data: renderActionMenu(resolveQuickRole(context), lang, context), action: 'menu' };
        case 'generate_platform_report':
            return executeTool('generate_report_file', { scope: 'platform', format: 'pdf' }, context);
        case 'system_health':
        ...
        case 'generate_mckinsey_report':
            return executeTool('generate_report_file', { scope: 'portfolio', format: 'pdf' }, context);
        ...
        case 'tenant_balance_inquiry':
            return executeTool('list_invoices', { scope: 'self', status: 'OPEN' }, context);
        case 'request_receipt':
            return executeTool('generate_receipt', { scope: 'last_payment' }, context);
        case 'report_maintenance':
            return executeTool('create_maintenance_request', {}, context);
        case 'request_statement':
            return executeTool('generate_report_file', { scope: 'tenant_statement', format: 'pdf' }, context);
        ...
        case 'CANCEL':
            return null;
        default:
            return { success: false, data: null, error: `Invalid action: ${action}`, action };
    }
};
...
export const tryDirectTool = async (
    ...
    const session = (await loadSession(cacheManager, context)) || { userId: context?.userId || 'anon' } as SessionState;
    ...
    const shouldSkipHijack = isDigit && listIntents.includes(session.lastIntent || '');
    ...
    if (shouldSkipHijack && session.lastResults) {
        ...
        if (result) {
            if (result.type === 'company') {
                ...
                return await executeTool('select_company', { companyId: result.id }, context);
            }
        }
    }

    // 0.5 YES/NO confirmation handler when a pending confirmation exists
    if (session.pendingConfirmation && session.pendingConfirmation.expiresAt > Date.now()) {
        const trimmed = message.trim();
        const numericKey = trimmed.replace('.', '');
        const optionAction = session.pendingConfirmation.options?.[numericKey];

        if (optionAction) {
            const followUp = await executeQuickAction(optionAction, context, executeTool, lang);
            session.pendingConfirmation = undefined;
            await saveSession(cacheManager, context, session);
            if (optionAction === 'CANCEL') return { success: true, data: lang === 'SW' ? '🚫 Imebatilishwa.' : '🚫 Cancelled.', action: 'cancel' };
            if (followUp) return followUp;
            return { success: true, data: lang === 'SW' ? '✅ Imefanikiwa.' : '✅ Done.', action: optionAction };
        }
        if (yesRegex.test(text)) {
            const followUp = await executeQuickAction(session.pendingConfirmation.action, context, executeTool, lang);
            session.pendingConfirmation = undefined;
            await saveSession(cacheManager, context, session);
            if (followUp) return followUp;
        ...
        if (noRegex.test(text)) {
            session.pendingConfirmation = undefined;
            await saveSession(cacheManager, context, session);
            return { success: true, data: lang === 'SW' ? '🚫 Imebatilishwa.' : '🚫 Cancelled.', action: 'cancel' };
        }
    }
    if (session.pendingConfirmation && session.pendingConfirmation.expiresAt <= Date.now()) {
        session.pendingConfirmation = undefined;
    ...
    if (storedMenuAction) {
        const quickResult = await executeQuickAction(storedMenuAction, context, executeTool, lang);
        
        // RELOAD session to catch updates from the tool (e.g. list_companies updates lastResults)
        const updatedSession = (await loadSession(cacheManager, context)) || session;
    ...
    const quickIntent = shouldSkipHijack ? null : routeQuickAction(message, quickRole);
    if (quickIntent) {
        const quickResult = await executeQuickAction(quickIntent, context, executeTool, lang);
        session.lastIntent = quickIntent;

        // Post-action suggestion hook
        const suggestionContext = typeof quickResult === 'object' && quickResult !== null
            ? { ...context, ...quickResult }
            : context;
        const suggestion = POST_ACTION_SUGGESTIONS[quickIntent]?.(lang, suggestionContext);
        if (suggestion) {
    ...
    if (greetableRegex.test(text)) {
        ...
        session.lastActionMenu = { role: quickRole, options: QUICK_ACTION_MAP[quickRole] };
        await saveSession(cacheManager, context, session);
        return { success: true, data: menu, action: 'menu' };
    }
    if (/^(help|msaada|what can you do|how to use)/i.test(text)) {
        return { success: true, data: renderTemplate('help', lang, {}), action: 'help' };
    }

    // 2. Vacancy Counts / Vacant Units
    if (/^how many (units are )?vacant|^vacancies|^list vacant units/i.test(text)) {
        return await executeTool('list_vacant_units', {}, context);
    }

    // 3. Company Summary
    if (/^company summary|^about (the |my )?company|^how is the business doing/i.test(text)) {
        if (!context.companyId) {
            return renderTemplate('company_selection_required', lang, {});
        }
        if ((context.role || context.userRole) === 'UNIDENTIFIED') {
            return { success: false, data: null, error: renderTemplate('unidentified_denial', lang, {}), action: 'get_company_summary' };
        }
    ...
    if (/^who am i|^my profile|^show my details/i.test(text)) {
        const companyInfo = context.companyId ? (lang === 'EN' ? ` at company ID ${context.companyId}` : ` katika kampuni ID ${context.companyId}`) : '';
        return {
            success: true,
    ...
    if (/^who hasn't paid|^arrears|^collection status|^unpaid units/i.test(text)) {
        if (!context.companyId) {
            return renderTemplate('company_selection_required', lang, {});
        }
    ...
    const hasPaymentSignal = /\bnimetuma\b|\bnimepay\b|\bi have paid\b|\bi've paid\b|\bmalipo\b|\bpaid\b/i.test(text);
