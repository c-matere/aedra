...
export const INTENT_TOOL_MAP: Record<string, string[]> = {
    ...
    // --- SHARED / PUBLIC ---
    'check_vacancy': ['list_vacant_units', 'get_unit_details', 'get_property_details'],
    'register_company': ['register_company'],

    // --- OPERATIONAL (STAFF/LANDLORD) ---
    'check_rent_status': ['get_portfolio_arrears', 'list_payments', 'list_invoices', 'list_tenants'],
    'arrears_check': ['get_portfolio_arrears', 'list_tenants', 'send_whatsapp_message'],
    'tenant_balance_inquiry': ['list_tenants', 'list_invoices', 'list_payments', 'get_tenant_details'],
    'get_tenant_details': ['search_tenants', 'get_tenant_details', 'list_leases', 'list_invoices', 'list_payments'],

    
    'log_maintenance': ['create_maintenance_request', 'list_properties', 'list_units', 'search_tenants'],
    'maintenance_request': ['create_maintenance_request', 'list_properties', 'list_units', 'search_tenants'],
    'report_maintenance': ['create_maintenance_request', 'list_properties', 'list_units', 'search_tenants'],
    'maintenance_status': ['list_maintenance_requests', 'update_maintenance_request', 'get_unit_details'],

    'send_single_reminder': ['search_tenants', 'send_whatsapp_message', 'get_tenant_details'],
    'send_bulk_reminder': ['get_portfolio_arrears', 'send_rent_reminders', 'send_whatsapp_message'],
    
    'record_payment': ['search_tenants', 'list_leases', 'record_payment', 'send_whatsapp_message', 'list_tenants'],
    'log_payment': ['search_tenants', 'list_leases', 'record_payment', 'send_whatsapp_message', 'list_tenants'],
    'nimetuma': ['search_tenants', 'list_leases', 'record_payment', 'send_whatsapp_message', 'list_tenants'],


    'request_receipt': ['list_payments', 'list_invoices', 'send_whatsapp_message', 'generate_report_file'],
    'add_tenant': ['create_tenant', 'list_properties', 'list_units', 'create_lease'],
    ...
    'yield_analysis': ['get_company_summary', 'get_financial_report', 'generate_report_file', 'list_properties'],

    'collection_status': ['get_company_summary', 'get_portfolio_arrears', 'get_financial_report'],
...
export const DEFAULT_TOOLS_BY_ROLE: Record<UserPersona, string[]> = {
    [UserPersona.TENANT]: [
        'list_vacant_units', 'list_maintenance_requests', 'create_maintenance_request', 
        'get_unit_details', 'list_invoices', 'list_payments'
    ],
    [UserPersona.LANDLORD]: [
        'list_properties', 'get_property_details', 'list_tenants', 'get_portfolio_arrears', 
...
export interface ConversationContext {
    ...
    lastEntityType?: 'tenant' | 'property' | 'unit' | 'company' | 'payment' | 'maintenance' | 'report';
    lastToolName?: string;
    userId?: string;
    companyId?: string;
    propertyId?: string;
 ...
 */
export function getSessionUid(context: { userId?: string; phone?: string } | any): string {
...
export const ENTITY_STICKY_TOOLS: Record<string, string[]> = {
    'tenant': ['get_tenant_details', 'record_payment', 'send_whatsapp_message', 'list_leases', 'list_invoices'],
    'property': ['get_property_details', 'list_units', 'list_tenants', 'get_company_summary'],
    'unit': ['get_unit_details', 'update_unit_status', 'create_lease', 'list_maintenance_requests'],
    'company': ['get_company_summary', 'list_properties', 'select_company'],
    'payment': ['list_payments', 'record_payment', 'send_whatsapp_message'],
    'maintenance': ['list_maintenance_requests', 'update_maintenance_request', 'create_maintenance_request'],
};

/**
 * TOOL_ENTITY_MAP: Maps specific tool names to the primary entity they manipulate.
 */
export const TOOL_ENTITY_MAP: Record<string, ConversationContext['lastEntityType']> = {
...
export function selectTools(
    ...
    const intentToolNames = INTENT_TOOL_MAP[intent] || DEFAULT_TOOLS_BY_ROLE[persona.id as UserPersona] || [];
    ...
    if (context?.lastToolName?.startsWith('search_') || context?.lastToolName?.startsWith('list_')) {
        contextTools.push('get_tenant_details', 'get_property_details', 'get_unit_details', 'get_lease_details');
    }

    // 4. Boost for common operational intents if not already present
    if (intent.includes('maintenance')) {
        contextTools.push('create_maintenance_request', 'list_maintenance_requests');
    }
    if (intent.includes('payment') || intent.includes('rent')) {
        contextTools.push('record_payment', 'list_payments', 'get_portfolio_arrears');
    }
    ...
    return allTools.filter(t => 
        requiredNames.has(t.name) && personaAllowed.has(t.name)
    );
}
