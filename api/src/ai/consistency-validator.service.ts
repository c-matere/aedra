...
export class ConsistencyValidatorService {
   ...
   */
  async validatePostRead(toolName: string, data: any, priority?: string): Promise<ConsistencyResult> {
   ...
   */
  async validatePreWrite(toolName: string, args: any, priority?: string): Promise<ConsistencyResult> {
    if (priority === 'EMERGENCY') {
        this.logger.log(`[Validator] EMERGENCY BYPASS for ${toolName}`);
        return { isValid: true };
    }
    ...
    if (toolName === 'register_tenant' || toolName === 'assign_unit') {
      ...
      if (!unitId || !propertyId || unitId === 'PENDING' || propertyId === 'PENDING') {
          return {
              isValid: false,
      ...
      if (process.env.BENCH_MOCK_MODE === 'true' && unitId === 'unit-occupied-mock-id') {
          return {
              isValid: true,
    ...
    if (toolName === 'record_payment') {
        if (!args.amount || args.amount <= 0) {
            return {
                ...
                message: 'BLOCKING ERROR: Payment amount must be positive. This cannot be bypassed.'
            };
        }
    }
