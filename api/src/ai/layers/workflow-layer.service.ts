...
import { ActionContract, AiIntent, Interpretation, ExecutionTrace } from '../ai-contracts.types';
import { WorkflowInstance, WorkflowStatus } from '../../workflows/workflow.types';
...
export class WorkflowLayer {
  ...
  async process(trace: ExecutionTrace): Promise<ExecutionTrace> {
    ...
    if (!interpretation || !actionContract) {
      this.logger.error(`[Workflow] Missing interpretation or contract in trace: ${trace.id}`);
      trace.status = 'FAILED';
      trace.errors.push('Missing interpretation or contract for workflow processing.');
      return trace;
    }

    this.logger.log(`[Workflow] Processing state for intent: ${interpretation.intent}`);
    ...
    if (instance && instance.status === 'WAITING') {
      ...
      const definition = findWorkflowByIntent(interpretation.intent.toLowerCase());
      if (definition) {
        this.logger.log(`[Workflow] Triggering new workflow: ${definition.id} for intent: ${interpretation.intent}`);
        instance = await this.workflowEngine.create(
          definition.id,
          userId,
          {
            chatId: sessionId,
    ...
    if (instance) {
      ...
      trace.metadata.activeIssueId = instance.context.maintenanceId || instance.context.issueId || trace.metadata.activeIssueId;
      trace.metadata.activeTenantId = instance.context.tenantId || trace.metadata.activeTenantId;
      trace.metadata.activeUnitId = instance.context.unitId || trace.metadata.activeUnitId;
      trace.metadata.activePropertyId = instance.context.propertyId || trace.metadata.activePropertyId;
      
      // Integrity Check: If workflow is RUNNING but required context is still missing, we might have a problem
      if (instance.status === 'RUNNING' && actionContract.requiresContext.some(c => !instance.context[c])) {
          this.logger.warn(`[Workflow] Trace ${trace.id} in RUNNING state but missing context fields: ${actionContract.requiresContext}`);
      }
    } else {
