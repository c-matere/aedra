...
export class IntegrityValidationLayer {
  ...
  validate(trace: ExecutionTrace): ExecutionTrace {
    this.logger.log(
      `[Integrity] Validating trace: ${trace.id} (Status: ${trace.status})`,
    );
    ...
    if (actionContract) {
      ...
      if (actionContract.requiresContext.length > 0) {
        const missingContext = actionContract.requiresContext.filter((c) => {
          const val =
            workflowState?.data?.[c] || truth?.context?.[c] || truthData[c];
          return (
            val === undefined || val === null || val === 'PENDING' || val === ''
          );
        });

        if (missingContext.length > 0) {
          this.logger.error(
            `[Integrity] Missing required context for ${actionContract.intent}: ${missingContext.join(', ')}`,
          );
      ...
      if (criteria?.mandatoryTools && criteria.mandatoryTools.length > 0) {
        ...
        if (failedTools.length > 0) {
          ...
          if (trace.status !== 'BLOCKED') {
            this.logger.error(
              `[Integrity] Mandatory tools failed/missing: ${failedTools.join(', ')}`,
            );
            trace.status = 'FAILED';
            trace.errors.push(
              `Mandatory tools failed: ${failedTools.join(', ')}`,
            );
            return trace;
          }
        }
      ...
      if (criteria?.requiredFields && criteria.requiredFields.length > 0) {
        ...
        if (missingFields.length > 0 && trace.status !== 'BLOCKED') {
          this.logger.error(
            `[Integrity] Required fields missing from truth: ${missingFields.join(', ')}`,
          );
          trace.status = 'FAILED';
          trace.errors.push(
            `Required data fields missing: ${missingFields.join(', ')}`,
          );
          return trace;
        }
      }
    ...
    if (truth) {
      ...
      if (
        illegalPatterns.some((p) => p.test(truthValues)) &&
        actionContract?.intent !== 'GENERAL_QUERY'
      ) {
        this.logger.error(
          `[Integrity] Hallucination/Incomplete data detected: ${truthValues}`,
        );
        trace.status = 'FAILED';
        trace.errors.push(
          'Incomplete or hallucinated data detected in final state.',
        );
        return trace;
      }
      ...
      const effectiveIntent =
        actionContract?.intent || trace.interpretation?.intent;
      if (effectiveIntent === AiIntent.PAYMENT_DECLARATION) {
        const truthAmount =
          truth.data?.amount || truth.data?.payment_amount || 0;
        ...
        if ((proposedAmount || entityAmount) && truthAmount === 0) {
          this.logger.error(
            `[Integrity] Proposed amount ${proposedAmount || entityAmount} not found in verified ledger. Blocking for confirmation.`,
          );
          trace.status = 'BLOCKED';
          trace.errors.push(
            `Value Verification Required: I see you've mentioned ${proposedAmount || entityAmount}, but I couldn't verify this in our ledger. Did I get the amount right?`,
          );
          return trace;
        }
        ...
        if ((proposedAmount || entityAmount) > 0 && truthAmount === 0) {
          this.logger.error(
            `[Integrity] Positive Grounding Failure: User claimed ${proposedAmount || entityAmount} but ledger shows 0.00.`,
          );
          trace.status = 'BLOCKED';
          trace.errors.push(
            `I've noted your amount of ${proposedAmount || entityAmount}. I'm waiting for the system to update our records—I'll confirm once it's reflected in the balance.`,
          );
          return trace;
        }
        ...
        if (!truthAmount && !entityAmount) {
          this.logger.error(
            `[Integrity] PAYMENT_DECLARATION missing amount. Blocking guess.`,
          );
          trace.status = 'BLOCKED';
          trace.errors.push(
            'Payment Amount Required: To record a payment, please specify the exact amount paid.',
          );
          return trace;
        }
      }
      ...
      const isFinancial =
        financialIntents.includes(actionContract?.intent || '') ||
        financialIntents.includes(trace.interpretation?.intent || '');
      ...
      if (isFinancial && !trace.errors.length) {
        ...
        const tenantId =
          truth.context?.tenantId || trace.interpretation?.entities?.tenantId;
        const hasData =
          (Array.isArray(revenue) && revenue.length > 0) ||
          typeof arrears === 'number' ||
          truth.data?.payments?.length > 0 ||
        ...
        if (!hasData || (!tenantId && !isReporting)) {
          this.logger.error(
            `[Integrity] ${effectiveIntent} blocked. Data missing or Identity not resolved.`,
          );
          trace.status = 'BLOCKED';
