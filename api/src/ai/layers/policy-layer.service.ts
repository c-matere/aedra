...
export class PolicyLayer {
  ...
  enforce(trace: ExecutionTrace): ExecutionTrace {
    ...
    if (!interpretation || !actionContract) {
      this.logger.error(
        `[Policy] Missing interpretation or contract in trace: ${trace.id}`,
      );
      trace.status = 'FAILED';
      trace.errors.push(
        'Missing interpretation or contract for policy enforcement.',
      );
      return trace;
    }

    this.logger.log(
      `[Policy] Enforcing laws for intent: ${interpretation.intent} (Role: ${role})`,
    );
    ...
    if (
      interpretation.intent === AiIntent.TENANT_COMPLAINT ||
      interpretation.intent === AiIntent.DISPUTE
    ) {
    ...
    const isEmergencyKeywords = interpretation.raw_reasoning
      ?.toLowerCase()
      .match(/\b(fire|flood|burst|moto|imepasuka)\b/);
    ...
    if (role === UserRole.TENANT) {
      ...
      if (
        actionContract.requiredTools.some((t) =>
          [
            'get_revenue_summary',
            'get_collection_rate',
            'list_properties',
        ...
        this.logger.warn(
          `[Policy] RBAC VIOLATION: Tenant attempted to access restricted tools.`,
        );
