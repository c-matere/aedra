...
export class LandlordIntentStrategy implements AiStrategy {
  ...
  async resolveIntent(message: string, history: any[], context: any): Promise<Partial<Interpretation>> {
    this.logger.log(`[LandlordStrategy] Resolving intent for message: ${message.substring(0, 30)}...`);
    const text = (message || '').toLowerCase();

    // 1. DETERMINISTIC ACTION GATING
    // Revenue / Collection
    if (/(revenue|collection|profit|income|mapato|makusanyo)/i.test(text)) {
      this.logger.log('[LandlordStrategy] Gating: Revenue/Collection Query detected.');
      return {
    ...
    const result = await this.classifier.classifyForRole(message, 'LANDLORD', context);
    ...
    if (Array.isArray(rawTruth.data)) {
        return {
            ...
            aggregates: {
                ...
                totalValue: (rawTruth.data as any[]).reduce((acc: number, curr: any) => acc + (curr.amount || 0), 0),
            },
