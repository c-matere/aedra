...
export class EmergencyEscalationService {
  private readonly ESCALATION_KEYWORDS = [
    'fire', 'moto', 'flood', 'mafuriko',
    'collapse', 'gesi', 'gas', 'umeme',
    'electrocution', 'damu', 'blood',
    'msaada', 'help me', 'emergency',
    'accident', 'injured', 'hurt'
  ];

  checkForEmergency(message: string): EmergencyCheckResult {
    const normalized = message.toLowerCase();
    const isEmergency = this.ESCALATION_KEYWORDS.some(k => normalized.includes(k));
