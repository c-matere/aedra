...
export class CriticService {
    ...
    private criticModelName = (process.env.GEMINI_MODEL || '').trim() || 'gemini-2.5-flash'; // Default to Gemini 2.5 Flash for critiquing
     ...
     */
    async evaluate(skillId: string, draftJson: string, context: string): Promise<CriticVerdict> {
        const skill = SKILLS_REGISTRY.find(s => s.skill_id === skillId);
        if (!skill) {
            return { pass: true, feedback: [] }; // Fallback
        }

        const model = this.genAI.getGenerativeModel({ model: this.criticModelName });
        
        const rubricList = skill.rubric.map((r: string, i: number) => `${i + 1}. ${r}`).join('\n');
...
`;
        ...
        try {
            ...
            this.logger.log(`Critic verdict for ${skillId}: ${verdict.pass ? 'PASS' : 'FAIL'}`);
            if (!verdict.pass) {
