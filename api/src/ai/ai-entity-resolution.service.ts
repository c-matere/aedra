...
export class AiEntityResolutionService {
  ...
  private async resolveProperty(
    ...
    if (matches.length === 1) {
      return { id: matches[0].id, match: matches[0], candidates: [], confidence: 0.9, mode: 'FUZZY' };
    }

    // In strict mode, force explicit disambiguation.
    if (strict) {
      return { id: null, match: null, candidates: matches.slice(0, 5), confidence: 0.6, mode: 'AMBIGUOUS' };
    }
    ...
    const top = scored[0] as any;
    const second = scored[1] as any;
    const isClearlyBest =
  ...
  private async resolveTenant(name: string, companyId?: string, unitHint?: string, strict: boolean = false): Promise<ResolutionResult> {
    ...
    if (normalizedPhone) {
      const tenant = await this.prisma.tenant.findFirst({
        where: { phone: { contains: normalizedPhone.slice(-9) }, companyId: cid },
        include: { leases: { include: { unit: true }, where: { status: 'ACTIVE', deletedAt: null } } }
      });
      if (tenant) return { id: tenant.id, match: tenant, candidates: [], confidence: 1.0, mode: 'EXACT' };
    }
        ...
        OR: [
          ...
          { 
            AND: [
              ...
              { lastName: { equals: q.split(/\s+/).slice(1).join(' ') || '', mode: 'insensitive' } }
            ]
          }
        ],
        companyId: cid 
    ...
    const isUnitLike = / unit /i.test(q) || (q.length <= 5 && /\d/.test(q)) || unitHint;
    const potentialUnit = unitHint || (isUnitLike ? q.replace(/unit/i, '').trim() : undefined);
    ...
    if (potentialUnit) {
      ...
      if (unitMatch && unitMatch.leases.length > 0) {
        ...
        if (q.toLowerCase().includes(unitMatch.unitNumber.toLowerCase()) && tenants.length === 1) {
    ...
    const scored = fuzzyMatches.map(t => {
      ...
      let score = 0;
      tokens.forEach(tok => {
        if (fullName.includes(tok.toLowerCase())) score += tok.length / q.length;
      });
    ...
    if (top._score >= 0.7 || (scored.length === 1 && top._score >= 0.4)) {
      return { id: top.id, match: top, candidates: [], confidence: 0.8 + (top._score * 0.15), mode: scored.length > 1 && scored[1]._score > top._score - 0.2 ? 'AMBIGUOUS' : 'FUZZY' };
    }
  ...
  private async resolveUnitRes(query: string, companyId?: string, strict: boolean = false): Promise<ResolutionResult> {
    ...
    const complexMatch = q.match(/^(.*?)\s+(?:unit|in|of)\s+([a-z0-9-]+)$/i) || q.match(/^unit\s+([a-z0-9-]+)\s+(?:in|of|at)\s+(.*)$/i);
    if (complexMatch) {
      const pName = q.match(/unit/i) ? (q.startsWith('unit') ? complexMatch[2] : complexMatch[1]) : complexMatch[1];
      const uNum = q.startsWith('unit') ? complexMatch[1] : complexMatch[2];
      
      const unit = await this.prisma.unit.findFirst({
        where: { 
          unitNumber: { equals: uNum, mode: 'insensitive' },
          property: { name: { contains: pName, mode: 'insensitive' }, companyId: cid }
        },
        include: { property: true }
      });
      if (unit) return { id: unit.id, match: unit, candidates: [], confidence: 0.98, mode: 'FUZZY' };
    }
    ...
    if (candidates.length > 1) {
       ...
       const exactInList = candidates.find(c => c.unitNumber.toLowerCase() === q.toLowerCase());
       if (exactInList) return { id: exactInList.id, match: exactInList, candidates: [], confidence: 0.95, mode: 'EXACT' };
       return { id: null, match: null, candidates, confidence: 0.7, mode: 'AMBIGUOUS' };
    }
