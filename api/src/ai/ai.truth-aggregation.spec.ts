...
describe('AiService Truth Aggregation', () => {
  it('picks report URL deterministically (prefer generate_report_file)', async () => {
    ...
    const trace: any = {
      ...
      unifiedPlan: { intent: AiIntent.FINANCIAL_REPORTING, entities: {}, steps: [] },
      steps: [
        { tool: 'download_report', success: true, result: { artifacts: [{ kind: 'report', url: 'https://example.com/old.pdf', fileName: 'old.pdf', format: 'pdf' }] } },
        { tool: 'some_other_tool', success: true, result: { url: 'https://example.com/aux.pdf' } },
        { tool: 'generate_report_file', success: true, result: { artifacts: [{ kind: 'report', url: 'https://example.com/new.pdf', fileName: 'new.pdf', format: 'pdf' }] } },
      ],
    };
