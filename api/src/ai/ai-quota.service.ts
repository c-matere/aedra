...
export class AiQuotaService {
  ...
  async updateUsage(tokens: number): Promise<number> {
    ...
    this.logger.log(
      `[Quota] Groq usage updated: +${tokens}. Total today: ${total}/${this.DAILY_GROQ_LIMIT}`,
    );
    return total;
  }
  ...
  async forceQuotaExceeded(): Promise<void> {
    const key = this.getDailyKey();
    this.logger.warn(
      `[Quota] System received a hard 429 from Groq. Locking Groq for the rest of the day.`,
    );
