...
export class AiClassifierService {
  ...
  async classify(
    ...
    this.logger.error(
      `All cloud classification tiers failed. Using local fallback.`,
    );
    return this.localClassify(message, role, undefined, attachmentsCount);
  }

  async classifyForRole(
    message: string,
    role: string,
    context?: any,
  ): Promise<ClassificationResult> {
    ...
    const allowedIntents = (
      (intentsByRole as any)[role] || ['general_query']
    ).join(', ');
    ...
    try {
      this.logger.log(
        `[Classifier] Classifying for ROLE: ${role} | intent space: [${allowedIntents}]`,
      );
      const isGemini = this.primaryModel.includes('gemini');
      let data: any;
      ...
      this.logger.warn(
        `[Classifier] Role-specific classification failed for ${role}: ${err.message}`,
      );
  ...
  private applyIntentGuardrails(
    ...
    const hasSystemFailureKeywords =
      /\bfail(ure|ed)?\b|\berror\b|\bbug\b|\bcrash\b/.test(text);
    ...
    if (
      (result.intent === 'add_tenant' ||
        result.intent === 'bulk_create_tenants') &&
      !hasTenantKeywords
    ) {
      if (
        isInterestInquiry &&
        (hasPropertyKeywords || hasSpecificPropertyRef)
      ) {
        return {
          ...result,
    ...
    const isEmergency =
      /pasuka|burst|flood|motto|fire|emergency|hatari|danger|short circuit/i.test(
        text,
      );
    if (
      isEmergency &&
      (result.intent === 'log_maintenance_issue' ||
        result.intent === 'general_query' ||
        result.intent === 'create_maintenance_request')
    ) {
      return {
        ...result,
    ...
    const isDomainProblem =
      /shida.*(kodi|nyumba|pesa|rent|arrears|receipt|balanc)/i.test(text) ||
      /(kodi|nyumba|pesa|rent|arrears|receipt|balanc).*shida/i.test(text);

    if (
      (hasSystemFailureKeywords ||
        this.systemFailureSignals.some((s) => s.test(text))) &&
      (result.intent === 'general_query' ||
        result.intent === 'system_failure') &&
      !isDomainProblem
    ) {
      return {
        ...result,
    ...
    const isPluralEntitySearch =
      /(?:tenants|members|people|guys|folks|ones|does|smiths|johns|marys)\b/i.test(
        text,
      );

    if (
      hasFinancialFigureRequest &&
      (result.intent === 'get_property_details' ||
        result.intent === 'general_query')
    ) {
      return {
        ...result,
  ...
  private localClassify(
    ...
    const hasTechnicalFailure =
      /fail|error|bug/.test(text) ||
      this.systemFailureSignals.some((s: RegExp) => s.test(text));
    ...
    if (
      /maintenance|tap|sink|bomba|imevunjika|leak|broken|repair|fix/.test(text)
    ) {
      const isWaterOut =
        /maji.*(imepotea|limepotea|lack|no water)/i.test(text) ||
        /no water/i.test(text);
    ...
    if (
      /noise|loud|disturb|shouting|neighbor|mpangaji wa|make kelele|kelele/.test(
        text,
      )
    ) {
    ...
    // Reports
    if (
      /full report|request report|send.*report|resend.*report|csv|pdf/.test(
        text,
      )
    ) {
    ...
    // Add tenant
    if (
      /add new tenant|create tenant|new tenant|register.*tenant/i.test(text)
    ) {
      const intent =
    ...
    if (
      /update property|feed.*data.*property|change property details/i.test(text)
    ) {
    ...
    if (
      /expense|cost|paid for|repair cost|bill|commission|fee|agent.*fee|management.*fee/i.test(
        text,
      )
    ) {
      const intent =
        text.includes('list') || text.includes('show') || text.includes('view')
          ? 'list_expenses'
          : 'record_expense';
      return finalize(
        this.result(
  ...
  private result(
    ...
    const confidence = reasonLower.includes('fallback')
      ? 0.35
      : reasonLower.includes('regex') ||
          reasonLower.includes('keyword') ||
          reasonLower.includes('signal') ||
          reasonLower.includes('intent')
        ? 0.9
        : 0.65;
