...
- Collection Rate: ${context.collectionRate || 0}%
  `;
    ...
    const registrationInfo =
      ...
      Object.keys(context.registrationData).length > 0
  ...
  async generateUnifiedPlan(
    ...
    const rolePrompt =
      role === UserRole.TENANT
        ? this.TENANT_AGENT_PROMPT
        : role === UserRole.LANDLORD
          ? this.LANDLORD_AGENT_PROMPT
          : role === UserRole.SUPER_ADMIN
            ? this.SUPER_ADMIN_AGENT_PROMPT
            : role === UserRole.UNIDENTIFIED
              ? this.UNIDENTIFIED_AGENT_PROMPT
...
- LastIdentities: ${JSON.stringify(context.lastIdentities || [])}
    ...
    try {
      ...
      return this.fallbackPlan(
        'I encountered a technical issue while planning. How else can I help?',
      );
    }
  }
  ...
  private validatePlan(raw: any): UnifiedPlan {
    ...
    this.logger.debug(
      `[UnifiedPlanner] Parsed ${steps.length} steps from raw plan.`,
    );
  ...
  async generateTakeoverAdvice(
    ...
    const safeContext = {
      ...
      activeTenantId:
        input.context?.activeTenantId || input.context?.tenantId || 'NONE',
      activeTenantName:
        input.context?.activeTenantName || input.context?.tenantName || 'NONE',
      activePropertyId:
        input.context?.activePropertyId || input.context?.propertyId || 'NONE',
      activeUnitId:
        input.context?.activeUnitId || input.context?.unitId || 'NONE',
      activeUnitNumber:
        input.context?.activeUnitNumber || input.context?.unitNumber || 'NONE',
      lastIntent:
        input.context?.lastIntent ||
        input.context?.lockedState?.lockedIntent ||
        'NONE',
    ...
    try {
      ...
      const suggestions = (
        ...
        .filter(
          (s: any) =>
            s && typeof s.label === 'string' && typeof s.tool === 'string',
        )
        .map((s: any) => ({
      ...
      this.logger.error(
        `[TakeoverAdvice] Advice generation failed: ${e.message}`,
      );
      const fallback =
        input.language === 'sw'
  ...
  public async generateFinalResponse(
    ...
    if (HIGH_STAKES_INTENTS.includes(intent)) {
      const allActionsSucceeded =
        (truthObject.actions?.length || 0) > 0 &&
        truthObject.actions!.every((a) => a.success);
      const hasData =
        truthObject.data?.balance !== undefined ||
        truthObject.data?.revenue !== undefined ||
        truthObject.data?.issueId !== undefined ||
        truthObject.data?.paymentHistory?.length > 0;

      if (
        truthObject.status === 'COMPLETE' &&
        (allActionsSucceeded || hasData)
      ) {
        return this.buildSuccessResponse(intent, truthObject, language);
      } else {
        return this.buildSafePartialResponse(
          intent,
          truthObject,
          language,
          immediateResponse,
  ...
  private buildSuccessResponse(
    ...
    if (intent === AiIntent.PAYMENT_PROMISE) {
      const toolAction = truth.actions?.find(
        (a) => a.tool === 'log_payment_promise',
      );
      if (
        toolAction?.result?.clarificationNeeded &&
        toolAction.result.message
      ) {
    ...
    if (
      ...
      const toolAction = truth.actions?.find(
        (a) =>
          a.tool === 'log_maintenance_issue' ||
          a.tool === 'create_maintenance_request',
      );
      if (
        toolAction?.result?.clarificationNeeded &&
        toolAction.result.message
    ...
    if (intent === AiIntent.ONBOARDING) {
      ...
      const propertyName =
        truth.data?.propertyName ||
        truth.data?.entities?.propertyName ||
        regData.propertyName ||
        regData.name ||
        'the property';
      const unitCount =
        truth.data?.unitCount ||
        truth.data?.entities?.unitCount ||
        regData.unitCount ||
        0;
      return isSw
    ...
    if (intent === AiIntent.FINANCIAL_QUERY) {
      ...
      const tenantName =
        truth.data?.searchedEntity?.name ||
        truth.data?.tenantIdentity?.name ||
        'the tenant';
    ...
    if (
      ...
      intent === AiIntent.REVENUE_REPORT
    ) {
  ...
  private buildSafePartialResponse(
    ...
    if (intent === AiIntent.FINANCIAL_QUERY) {
      const entity =
        truth.data?.searchedEntity?.name ||
        truth.data?.entities?.tenantName ||
        'the tenant';
    ...
    if (intent === AiIntent.ONBOARDING) {
      ...
      const missing = [];
      if (!regData.propertyName && !regData.name)
        missing.push(isSw ? 'jina la jengo' : 'property name');
      if (!regData.propertyAddress && !regData.address)
        missing.push(isSw ? 'anwani' : 'address');
  ...
  private async safeLlmRender(
    ...
    const publicSteps = (steps || [])
      ...
      .map((s) => ({
        tool: s.tool,
        summary: this.sanitizeForPublic(
          s.result?.summary || s.result?.status || 'Completed',
        ),
      }));
    ...
    if (intent === AiIntent.EMERGENCY || truthObject.data?.isEmergency) {
      ...
      intent === AiIntent.UTILITY_OUTAGE ||
      truthObject.data?.isUtilityOutage
    ) {
      emergencyInstructions = `
    ...
    try {
      ...
      try {
        ...
        this.logger.error(
          `[PromptService] Fallback rendering also failed: ${fallbackError.message}`,
        );
  ...
  public async generateNativeFunctionCall(
    ...
    const tools = AI_TOOL_DEFINITIONS.filter((t) =>
...
Context: ${JSON.stringify(context)}
  ...
  private async callLLMWithFailover(
    ...
    try {
      ...
      if (isNetworkError || this.isRateLimitError(e)) {
        this.logger.warn(
          `[Failover] Primary model (${modelName}) failed with ${isNetworkError ? 'network' : 'rate limit'} error. Switching to Groq (${this.groqModel})...`,
        );
        try {
          ...
          this.logger.error(
            `[Failover] Groq also failed: ${groqError.message}`,
          );
          throw groqError;
        }
      }
  ...
  private async callLLMWithRetry(
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        this.logger.warn(
          `[LLM] ${modelName} attempt ${attempt + 1}/${maxRetries + 1} failed: ${msg}`,
        );
  ...
  private sanitizeForPublic(obj: any): any {
    ...
    if (Array.isArray(obj))
      return obj.map((item) => this.sanitizeForPublic(item));
