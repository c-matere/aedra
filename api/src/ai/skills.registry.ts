...
export const SKILLS_REGISTRY: AedraSkill[] = [
  {
    ...
    system_prompt_injection: 'You are helping a property manager check rent payment status. Be precise and mention amounts and dates clearly.',
  ...
  },
  {
    ...
    description: 'Sends WhatsApp reminders to tenants with outstanding balances.',
    ...
    system_prompt_injection: 'You are assisting with sending rent reminders. Ensure the tone is professional yet firm. Use the correct tenant names.',
    tools_required: ['send_whatsapp_message', 'list_tenants', 'list_leases', 'list_invoices'],
    outputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    trigger_intents: ['log_maintenance', 'report_maintenance', 'maintenance_request'],
    tier_required: 1,
    persona_id: UserPersona.TENANT,
    objective: 'Log new maintenance issues with accurate categories and descriptions.',
    system_prompt_injection: 'You are logging a maintenance request. Categorize it correctly (PLUMBING, ELECTRICAL, etc.) based on user description. Be empathetic.',
    tools_required: ['create_maintenance_request', 'list_properties', 'list_units'],
    outputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Generates high-level McKinsey-style portfolio intelligence reports.',
    trigger_intents: ['generate_mckinsey_report', 'report_generation'],
    tier_required: 3,
    persona_id: UserPersona.LANDLORD,
    objective: 'Generate a comprehensive portfolio intelligence report with ZERO redundant questioning. Assume Summary PDF if vague.',
    ...
    sw_vocabulary: ['kimkakati', 'faida', 'ukaliaji', 'mali', 'tathmini ya hatari'],
    language_variants: {
  ...
  },
  {
    ...
    trigger_intents: ['record_payment', 'payment_record', 'log_payment', 'nimetuma'],
    tier_required: 2,
    persona_id: UserPersona.STAFF,
    objective: 'Record payment accurately against the correct tenant and lease. Prevent duplicates.',
    system_prompt_injection: 'You are recording a rent payment. Match the tenant by phone or name. Confirm the amount and date before recording. Check for duplicates.',
    tools_required: ['record_payment', 'list_tenants', 'list_leases', 'list_payments'],
    outputSchema: {
      type: 'object',
      properties: {
    ...
    sw_vocabulary: ['malipo', 'lipa', 'ametuma', 'stakabadhi', 'risiti', 'mpesa'],
    language_variants: {
  ...
  },
  {
    ...
    description: 'Creates a new property with units from a spreadsheet, image, or description.',
    trigger_intents: ['onboard_property', 'add_property', 'create_property'],
    tier_required: 2,
    persona_id: UserPersona.STAFF,
    objective: 'Create property and all units in a single turn with minimum user input.',
    system_prompt_injection: 'You are onboarding a new property. Extract name, address, and unit list from the user or attachment. Create property first, then units in sequence. Summarize what was created.',
  ...
  },
  {
    ...
    system_prompt_injection: 'You are helping find vacant rental units. List all vacant units with rent amount and location. Be welcoming and clear for potential tenants.',
    ...
    sw_vocabulary: ['nyumba', 'inapatikana', 'kodi ya kila mwezi', 'eneo', 'chumba cha kukodi'],
    language_variants: {
  ...
  },
  {
    ...
    system_prompt_injection: 'You are onboarding a new tenant. Collect: full name, phone, unit. Optionally: lease start date and monthly rent. Create tenant, then lease. Use data from images or documents if provided.',
    ...
    sw_vocabulary: ['mpangaji', 'andika', 'mkataba', 'nambari ya simu', 'tarehe ya kuanza'],
    language_variants: {
  ...
  },
  {
    ...
    description: 'Gives a tenant their own current balance, lease status, and next due date.',
    trigger_intents: ['tenant_balance_inquiry', 'my_balance', 'check_my_balance', 'balance_inquiry'],
    tier_required: 1,
    persona_id: UserPersona.TENANT,
    objective: 'Give the tenant a clear, empathetic summary of their own account balance.',
    system_prompt_injection: 'You are helping a tenant understand their rent balance. Show: current balance, last payment date, next due date. Be encouraging if paid, empathetic if overdue.',
    ...
    sw_vocabulary: ['salio langu', 'nilipewa', 'tarehe ya kulipa', 'deni', 'nimelipa'],
    language_variants: {
...
export function getSkillByIntent(intent: string): AedraSkill | undefined {
  return SKILLS_REGISTRY.find(skill => skill.trigger_intents.includes(intent));
}
