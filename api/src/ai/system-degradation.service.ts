...
export type DegradationStatus = 'HEALTHY' | 'DEGRADED_CACHE' | 'DEGRADED_LLM_FALLBACK';
...
export class SystemDegradationService {
        ...
        switch (type) {
            case 'CACHE_DOWN':
                if (this.currentStatus !== 'DEGRADED_CACHE') {
                    this.logger.warn(`[SystemMode] Degradation detected: Redis cache unavailable. In-memory fallback active.`);
            ...
            case 'GEMINI_FAIL':
                if (this.currentStatus !== 'DEGRADED_LLM_FALLBACK') {
                    this.logger.warn(`[SystemMode] Degradation detected: Primary LLM API failure (${type}). Fallback active.`);
    ...
    reset() {
        if (this.currentStatus !== 'HEALTHY') {
            this.logger.log('[SystemMode] Resetting degradation status to HEALTHY after successful response.');
            this.currentStatus = 'HEALTHY';
        }
    }
