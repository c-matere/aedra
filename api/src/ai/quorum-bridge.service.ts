...
export class QuorumBridgeService {
     ...
     */
    async evaluateAction(toolName: string, args: any, userId: string): Promise<{ authorized: boolean; actionId?: string; message?: string }> {
        const config = SENSITIVE_ACTIONS_REGISTRY[toolName];
        if (!config || config.quorumRequired <= 1) {
        ...
        let request = pending.find(r => JSON.stringify(r.payload) === JSON.stringify(args));
        ...
        if (!request) {
            ...
            this.logger.log(`Created new quorum request ${request.id} for ${toolName}`);
        }
