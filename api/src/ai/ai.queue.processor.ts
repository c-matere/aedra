...
export class AiQueueProcessor extends WorkerHost {
        ...
        try {
            ...
            if (job.name === 'generate_report_pdf' && job.data?.targetPhone) {
                try {
                    ...
                    this.logger.error(`Failed to send background error feedback: ${sendError.message}`);
                }
            }
    ...
    private async handleGenerateReportPdf(data: {
        ...
        if (data.filters?.propertyId) {
            reportData = await this.reportsService.getPortfolioData(data.filters.propertyId, actor as any, start, end);
        } else {
            ...
            reportData = {
                ...
                totals: {
                    occupancy: Math.round((occupancy.OCCUPIED / (occupancy.VACANT + occupancy.OCCUPIED || 1)) * 100),
        ...
        const insights = await this.reportIntelligence.generatePremiumInsights(reportData);

        this.logger.log(`[Queue] Rendering PDF...`);
        const url = await this.reportsGenerator.generatePremiumPdf(insights, reportData, fileName);
        ...
        this.logger.log(`[Queue] PDF generated and delivered to ${data.targetPhone}`);
    }

    private async handleBulkInvoices(data: { companyId: string, propertyId: string, month: number, year: number, adminPhone: string }) {
        // Mock bulk generation logic that would otherwise block the AI loop for 30s+
        this.logger.log(`[Queue] Generating bulk invoices for property ${data.propertyId}`);
