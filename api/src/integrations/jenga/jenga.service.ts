...
export class JengaService {
  ...
  private async getConnector(companyId: string): Promise<JengaConnector> {
    ...
    if (!company || !company.jengaEnabled) {
      throw new NotFoundException(`Jenga is not enabled or configured for company ${companyId}`);
    }

    // Decrypt sensitive fields
    const sensitiveFields = ['jengaMerchantCode', 'jengaConsumerSecret', 'jengaApiKey', 'jengaPrivateKey'];
    const decrypted = this.vaultService.decryptObject(company, sensitiveFields);

    if (!decrypted.jengaMerchantCode || !decrypted.jengaApiKey || !decrypted.jengaPrivateKey) {
        throw new Error(`Incomplete Jenga configuration for company ${companyId}`);
    }
  ...
  async initiatePayment(invoiceId: string) {
    ...
    if (!invoice.lease.tenant.phone) throw new Error('Tenant has no phone number');
    ...
    try {
      this.logger.log(`Initiating Jenga STK Push for company ${companyId}, invoice ${invoiceId} to ${phone}`);
      ...
      this.logger.error(`Error in Jenga STK Push (Company: ${companyId}): ${error.message}`);
      throw new InternalServerErrorException(`Failed to initiate Jenga payment: ${error.message}`);
    }
  }
  ...
  async reconcilePayment(companyId: string, payload: any) {
    this.logger.log(`Attempting reconciliation for company ${companyId}: ${payload.transaction?.reference}`);
    ...
    if (!invoice && payerPhone) {
        this.logger.log(`Reference match failed for ${reference}. Falling back to phone lookup: ${payerPhone}`);
        ...
        if (tenant && tenant.leases.length > 0 && tenant.leases[0].invoices.length > 0) {
            invoice = tenant.leases[0].invoices[0] as any;
            this.logger.log(`Found pending invoice ${invoice?.id} for tenant ${tenant.id} via phone match.`);
        }
    }

    if (!invoice) {
        this.logger.warn(`Could not reconcile payment ${payload.transactionId} for company ${companyId}. No matching invoice/tenant found.`);
        return { reconciled: false, reason: 'No matching invoice or tenant found' };
    }
