...
export class ZuriLeaseConnector implements IConnector {
      ...
      try {
        ...
        if (this.page)
          await this.page.screenshot({
            path: `/home/chris/aedra/api/login_failed.png`,
          });
        throw e;
      }
    }
  ...
  async listProperties(): Promise<string[]> {
    ...
    const scanPage = async () => {
      const found = await this.page!.evaluate(() => {
        const links = Array.from(
          document.querySelectorAll('a, option, .nav-link'),
        );
    ...
    if (ids.size === 0) {
      console.log(
        '[Discovery] No properties found on dashboard. Navigating to /SelectProperty...',
      );
      await this.page
        .goto(this.getUrl('/SelectProperty'), {
          waitUntil: 'networkidle2',
          timeout: 30000,
        })
        .catch((e) =>
          console.warn(
            `[Discovery] SelectProperty navigation failed: ${e.message}`,
          ),
        );
    ...
    if (ids.size === 0) {
      const breadcrumbId = await this.page.evaluate(() => {
        const breadcrumbs = Array.from(
          document.querySelectorAll('.breadcrumb-item, .breadcrumb a'),
        );
        for (const b of breadcrumbs) {
          const href = b.getAttribute('href');
          if (href && href.includes('property_id=')) {
            try {
              const id = new URL(href, window.location.origin).searchParams.get(
                'property_id',
              );
              if (id) return id;
            } catch (e) {}
    ...
    if (ids.size === 0) {
      console.log(
        '[Discovery] Trying strategy 5: Explicit Search via portfolio_dashboard.jsp...',
      );
      try {
        await this.page.goto(this.getUrl('/portfolio_dashboard.jsp'), {
          waitUntil: 'networkidle2',
          timeout: 30000,
        });

        const hasSearch = await this.page.evaluate(
          () => !!document.querySelector('#propertySearchInput'),
        );
        if (hasSearch) {
          ...
          console.log(
            '[Discovery] Property search input not found on portfolio dashboard.',
          );
        }
      } catch (e) {
    ...
    if (ids.size === 0) {
      ...
      try {
        await this.page.goto(this.getUrl('/LoadListOfLandlords'), {
          waitUntil: 'networkidle2',
          timeout: 30000,
        });
        await scanPage();
    ...
    console.log(
      `[Discovery] Final discovery results: ${finalIds.join(', ')} (${finalIds.length} found)`,
    );
    return finalIds;
  }
  ...
  async fetchData(params: { propertyId: string }): Promise<ZuriLeaseData> {
    ...
    if (!property.code && !property.alias) {
      console.warn(
        `Property ${params.propertyId} seems inaccessible or empty. Skipping unit/payment scrape.`,
      );
    ...
    try {
      ...
      console.warn(
        `Could not fetch remittances for property ${params.propertyId}: ${e.message}`,
      );
    }
    ...
    for (let i = 0; i < unitsToProcess.length; i += concurrencyLimit) {
      ...
      await Promise.all(
        chunk.map(async (unit) => {
          unit.leases = await this.fetchUnitLeases(
            params.propertyId,
            unit.unitId,
          );
        }),
    ...
    if (tenantIds.length > 0) {
      try {
        console.log(
          `Performing reporting session warmup with tenant ${tenantIds[0]}...`,
        );
    ...
    for (const tid of tenantIds) {
      try {
        ...
        try {
          ...
          console.warn(
            `Could not fetch statement for tenant ${tid}: ${e.message}`,
          );
        }
  ...
  private async warmupReportingSession(tenantId: string): Promise<void> {
    ...
    console.log(
      '--- Establishing reporting session via official Sidebar path ---',
    );
    await this.page.goto(this.getUrl('/index.jsp'), {
      waitUntil: 'networkidle2',
    });
    ...
    await this.page.evaluate(() => {
      ...
      const tenantsTab = sidebarLinks.find(
        (l) => (l.textContent || '').trim().toUpperCase() === 'TENANTS',
      );
    ...
    const billingFound = await this.page.evaluate(() => {
      ...
      const billingLink = links.find(
        (el) => (el.textContent || '').trim() === 'Billing',
      );
      if (billingLink) {
  ...
  private async fetchTenantStatement(
    ...
    try {
      ...
      try {
        await downloadPage.goto(reportUrl, {
          waitUntil: 'networkidle2',
          timeout: 30000,
        });
      } catch (e: any) {
      ...
      for (let i = 0; i < 15; i++) {
        ...
        const reportFile = files.find(
          (f) =>
            f.includes('TenantAccountingStatement') &&
      ...
      rows.forEach(function (cols) {
        ...
        const dateIdx = cols.findIndex((c) =>
          /^\d{1,2}-[A-Za-z]{3,9}-\d{4}$/.test(c.trim()),
        );
        if (dateIdx === -1) return;
        ...
        const codeIdx = cols.findIndex(
          (c, i) =>
            i > dateIdx &&
            /INV|RCT|BILL|PAY|SI\d+|SJ\d+|RCT/.test(c.toUpperCase()),
        );
        if (codeIdx === -1) return;
        ...
        if (
          code.toUpperCase().includes('INV') ||
          code.toUpperCase().includes('BILL')
        ) {
          invoices.push({
            code: code,
            date: date,
          ...
          code.toUpperCase().includes('RCT') ||
          code.toUpperCase().includes('PAY')
        ) {
          receipts.push({
            code: code,
            date: date,
      ...
      console.error(
        `Error fetching tenant statement for ${tenantId}:`,
        e.message,
      );
      return { receipts: [], invoices: [] };
    }
  ...
  private async navigateToTenants(): Promise<void> {
    ...
    await this.page.evaluate(() => {
      const links = Array.from(
        document.querySelectorAll('a, button, li, .nav-link'),
      );
  ...
  private async fetchPropertyDetails(
    propertyId: string,
  ): Promise<ZuriLeaseProperty> {
    if (!this.page) throw new Error('Connector not connected');

    await this.page.goto(
      this.getUrl(`/SelectProperty?property_id=${propertyId}`),
      {
  ...
  private async fetchRemittances(
    ...
    await new Promise((r) => setTimeout(r, 2000));
    await this.page
      .waitForSelector('#remittances, table', { timeout: 8000 })
      .catch(() => {});
  ...
  private async fetchUnitLeases(
    propertyId: string,
    unitId: string,
  ): Promise<ZuriLeaseLease[]> {
    ...
    try {
      ...
      await unitPage
        .waitForSelector('#tenant', { timeout: 5000 })
        .catch(() => {});
      ...
      console.error(
        `Error fetching unit leases for unit ${unitId}:`,
        e.message,
      );
      return [];
    } finally {
  ...
  private async fetchUnits(propertyId: string): Promise<ZuriLeaseUnit[]> {
    ...
    await this.page
      .waitForSelector('#units, #tab-all-units, #tab-listing', {
        timeout: 8000,
      })
      .catch(() => {
        console.warn(
          'Units tab pane did not become active via selector. Proceeding with caution.',
        );
      });
      ...
      await this.page.waitForFunction(
        () => {
          ...
          const table = tables.find((t) => {
            ...
            return (
              text.includes('Code') &&
              (text.includes('Unit Type') || text.includes('Occupancy'))
            );
          });
          if (!table) return false;
          const dataRows = Array.from(table.querySelectorAll('tbody tr'));
          return (
            dataRows.length > 0 && dataRows[0].querySelectorAll('td').length > 1
          );
        },
        { timeout: 15000 },
      );
  ...
  private async fetchTenantDetails(
    tenantId: string,
  ): Promise<Partial<ZuriLeaseTenant> | null> {
    ...
    try {
      ...
      await tenantPage.goto(
        this.getUrl(`/SelectTenant?tenant_id=${tenantId}`),
        {
          waitUntil: 'domcontentloaded',
          timeout: 90000,
        },
      ...
      console.error(
        `Error fetching tenant details for ${tenantId}:`,
        e.message,
      );
      return null;
    } finally {
  ...
  private async fetchTenantPayments(
    tenantId: string,
  ): Promise<ZuriLeaseReceipt[]> {
    if (!this.page || !this.browser) return [];
    ...
    try {
      ...
      await tenantPage
        .waitForSelector('#receipts, table', { timeout: 8000 })
        .catch(() => {});
      ...
      console.error(
        `Error fetching tenant payments for ${tenantId}:`,
        e.message,
      );
      return [];
    } finally {
  ...
  private async fetchTenantInvoices(
    tenantId: string,
  ): Promise<ZuriLeaseInvoice[]> {
    if (!this.page || !this.browser) return [];
    ...
    try {
      ...
      await tenantPage
        .waitForSelector('#invoices, #bills, #billHistory, table', {
          timeout: 8000,
        })
        .catch(() => {});
      ...
      console.error(
        `Error fetching tenant invoices for ${tenantId}:`,
        e.message,
      );
      return [];
    } finally {
