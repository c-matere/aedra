...
export const routeWorkflowRequest = async (
    ...
    if (active && active.status === 'WAITING') {
        console.log(`[WorkflowRouter] Resuming active workflow ${active.workflowId} for user ${userId}`);
        return engine.resume(active.instanceId, { type: 'USER_MESSAGE', content: opts.message });
    }

    // Path 2 — matched workflow by intent
    console.log(`[WorkflowRouter] Checking for workflow match with intent: ${intent}`);
    const matched = findWorkflowByIntent(intent);
    if (matched) {
        console.log(`[WorkflowRouter] Starting NEW workflow ${matched.id} for intent ${intent}`);
