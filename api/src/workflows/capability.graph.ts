...
export const toolsThatConsume = (shape: string, graph: CapabilityNode[] = CAPABILITY_GRAPH): CapabilityNode[] =>
    graph.filter(node => node.consumes.includes(shape));

export const toolsThatProduce = (shape: string, graph: CapabilityNode[] = CAPABILITY_GRAPH): CapabilityNode[] =>
    graph.filter(node => node.produces.includes(shape));
...
export const shortlistTools = (
    ...
    const { consumes = [], produces = [], requires_state = [], tags = [], max = 5 } = params;

    const matches = graph.filter(node => {
        const consumesOk = consumes.length === 0 || consumes.every(c => node.consumes.includes(c));
        const producesOk = produces.length === 0 || produces.every(p => node.produces.includes(p));
        const stateOk = requires_state.length === 0 || requires_state.every(s => node.requires_state.includes(s));
        const tagsOk = tags.length === 0 || tags.some(tag => node.tags.includes(tag));
